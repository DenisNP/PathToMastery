...
namespace PathToMastery.Services
{
    public class PathService
    {
        ...
        public PathService(IDbService dbService)
        {
        ...
        private Path GenerateCalendar(PathData data, int offset)
        {
            ...
            var min = ToClosestDown(now - TimeSpan.FromDays(30), 1);
            var max = ToClosestUp(now + TimeSpan.FromDays(60), 7);

            // extend edges if there are done days
            if (data.Done.Count > 0)
            {
                var first = ToClosestDown(data.Done.First().ToDateTimeOffset(offset) - TimeSpan.FromDays(15), 1);
                if (first < min) min = first;

                var last = ToClosestUp(data.Done.Last().ToDateTimeOffset(offset) + TimeSpan.FromDays(45), 7);
